...
header("location: support_name_change.php?noPermission=1");

} else {


    $sqlciv = "UPDATE core_members SET name='".$newName."' WHERE member_id=".$identity.";";

    if ($conn->query($sqlciv) === TRUE) {


     $sqlLog = "INSERT INTO panel_logs VALUES (NULL, '".$userExecute." Set ".$identity." Forum Name to ".$newName."', '$insertUrl');";

        if ($conn->query($sqlLog) === TRUE) {
          echo "successful";
        header("location: support_name_change.php?successful=1");
