<?php
 ...
 if (isset($_POST["schange2"]))
 {
     $group = $_POST["schange2"];
 } else {
   $group = "0";
 }
 ...
 header("location: user_forums.php?noPermission=1");
   ...
   foreach ($_POST['schange2'] as $selected_option) {
     $secondaryGroups[] = $selected_option;
   }

   $str = implode (",", $secondaryGroups);

     $sqlSecondary = "UPDATE core_members SET mgroup_others='".$str."' WHERE member_id=".$identity.";";

     if ($conn->query($sqlSecondary) === TRUE) {


      $sqlLog = "INSERT INTO panel_logs VALUES (NULL, '".$userExecute." Changed ".$identity." Secondary Groups to ".$str."', NULL);";
